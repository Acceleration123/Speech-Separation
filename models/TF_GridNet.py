...
config = toml.load("/configs/configure_tfgridnet.toml")
...
class TF_GridNet(nn.Module):
    def __init__(self):
        ...
        self.num_blks = config["TF-GridNet"]["num_TF-Grid_blocks"]
        ...
        self.window = torch.hann_window(window_length=config["FFT"]["win_length"]).pow(0.5)

        self.stft_encoder = STFT_Encoder(n_fft=config["FFT"]["n_fft"],
                                         hop_length=config["FFT"]["hop_length"], window=self.window)

        self.conv = nn.Conv2d(in_channels=2, out_channels=self.embedding, kernel_size=(3, 3),
                              stride=(1, 1), padding=(1, 1))
        self.gln = nn.GroupNorm(num_groups=1, num_channels=self.embedding, eps=1e-8)

        self.tf_grid_blks = nn.ModuleList([])
        for i in range(self.num_blks):
            self.tf_grid_blks.append(GridNetBlock(emb_dim=self.embedding, emb_ks=self.kernel_size, emb_hs=self.stride,
                                                  n_freqs=config["TF-Grid_Block"]["width"],
                                                  hidden_channels=self.hidden_size))

        self.de_conv = nn.ConvTranspose2d(in_channels=self.embedding, out_channels=4, kernel_size=(3, 3),
                                          stride=(1, 1), padding=(1, 1))

        self.stft_decoder = STFT_Decoder(n_fft=config["FFT"]["n_fft"],
                                         hop_length=config["FFT"]["hop_length"], window=self.window)
